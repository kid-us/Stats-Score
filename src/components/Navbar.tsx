...
const Navbar = () => {
  ...
  return (
    <header>
      <nav className="h-[60px] flex justify-between items-center px-4 opacity-100 bg-foreground font-poppins! overflow-hidden">
        {/* Logo */}
        <Link to={"/"}>
          <div className="w-[82px] h-[26.1px] md:h-[50px] lg:h-[60px] md:w-[140px] lg:w-[200px]">
        ...
        <div className="flex items-center gap-4">
          ...
          {nav.map((n) => (
            <NavLink
              ...
              className={({ isActive }) =>
                `${
                  width < 1370 ? "hidden" : "block"
                }  decoration-2 decoration-primary underline-offset-8 text-[16px] p-2 ${
          ...
          <div className="bg-accent flex items-center justify-center lg:w-[129px] lg:h-10 rounded-full hover:bg-background/80 transition-colors duration-300 hover:border hover:border-zinc-400 ease-in px-1">
            <Select value={selectedSeason} onValueChange={setSelectedSeason}>
              <SelectTrigger className="w-full rounded-full border-none text-white [&>svg]:text-white! focus:ring-0! focus:ring-offset-0! focus:outline-none! text-xs md:text-sm lg:text-sm md:gap-1! gap-0! font-extralight!">
          ...
          {/* Language Icon */}
          <div className="hidden md:flex items-center justify-center w-6 h-6 md:w-8 md:h-8 lg:w-10 lg:h-10 rounded-full bg-accent">
            <img
              src={englandFlag}
              className="lg:w-6 md:w-5 w-4 lg:h-6 md:h-5 h-4"
              alt="Language"
      ...
      <div
        className={`${
          width > 768 && width < 1370 ? "flex" : "hidden"
        } justify-center space-x-5 bg-foreground border-t border-muted py-1`}
      >
        {nav.map((n) => (
          <NavLink
            to={n.path}
            key={n.id}
            className={({ isActive }) =>
              `decoration-2 decoration-primary underline-offset-8 text-[15px] p-2 ${
                isActive ? "underline text-primary" : "hover:underline"
              }`
            }
          >
